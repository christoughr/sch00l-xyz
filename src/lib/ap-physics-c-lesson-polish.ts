/** Sort and polish AP Physics C publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/electric|magnetic|gauss|capacitor|induction/i.test(t)) return 4;
  if (/torque|rotation|harmonic|angular/i.test(t)) return 3;
  if (/momentum|energy|work|collision|integral/i.test(t)) return 2;
  return 1;
}
