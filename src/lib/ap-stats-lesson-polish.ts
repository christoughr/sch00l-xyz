/** Sort and polish AP Statistics publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/regression|correlation|chi-square|bivariate|r-squared/i.test(t))
    return 5;
  if (/inference|confidence interval|hypothesis test|p-value/i.test(t))
    return 4;
  if (/probability|random variable|normal model|binomial/i.test(t)) return 3;
  if (/sampling|experiment|bias|simulation|random sample/i.test(t)) return 2;
  if (/distribution|histogram|boxplot|mean|median|outlier/i.test(t))
    return 1;
  return 1;
}
