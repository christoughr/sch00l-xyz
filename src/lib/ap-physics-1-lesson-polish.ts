/** Sort and polish AP Physics 1 publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/circuit|ohm|resistance|coulomb|electric field/i.test(t)) return 4;
  if (/torque|rotation|harmonic|wave|pendulum/i.test(t)) return 3;
  if (/momentum|energy|work|collision/i.test(t)) return 2;
  return 1;
}
