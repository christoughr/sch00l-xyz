/** Sort and polish College General Chemistry I publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["unit", "sig fig", "mole", "molar mass", "measurement", "dimensional", "density"],
  2: ["electron", "orbital", "periodic", "ionization", "configuration", "nomenclature", "ion"],
  3: ["lewis", "vsepr", "bond", "polar", "intermolecular", "geometry", "hybrid", "dipole"],
  4: ["stoichiometry", "balance", "limiting", "reaction", "molarity", "titration", "yield", "solution"],
  5: ["enthalpy", "calorimetry", "gas law", "ideal gas", "thermochemistry", "hess", "review"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/enthalpy|gas law|thermochem/i.test(t)) return 5;
  if (/stoichiometry|reaction|molarity|titration/i.test(t)) return 4;
  if (/lewis|bond|vsepr|intermolecular/i.test(t)) return 3;
  if (/electron|periodic|orbital/i.test(t)) return 2;
  return 1;
}
