/** Sort and polish College Physics I publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["kinematics", "velocity", "acceleration", "projectile", "vector", "motion", "graph"],
  2: ["force", "newton", "friction", "incline", "free-body", "circular", "centripetal", "normal"],
  3: ["energy", "work", "power", "momentum", "collision", "impulse", "conservation", "kinetic"],
  4: ["torque", "rotation", "angular", "harmonic", "oscillation", "pendulum", "spring", "rolling"],
  5: ["fluid", "pressure", "bernoulli", "buoyancy", "pascal", "review", "synthesis"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/fluid|pressure|bernoulli/i.test(t)) return 5;
  if (/torque|rotation|oscillation|harmonic/i.test(t)) return 4;
  if (/energy|momentum|collision|work/i.test(t)) return 3;
  if (/force|newton|friction/i.test(t)) return 2;
  return 1;
}
