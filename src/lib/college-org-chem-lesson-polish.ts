/** Sort and polish Organic Chemistry publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["hybrid", "bond", "resonance", "lewis", "orbital", "acid", "base", "structure", "functional", "pka"],
  2: ["alkane", "cycloalkane", "conformation", "stereo", "chiral", "enantiomer", "configuration", "newman", "r/s"],
  3: ["sn1", "sn2", "e1", "e2", "substitution", "elimination", "nucleophil", "leaving", "carbocation"],
  4: ["alkene", "addition", "markovnikov", "alkyne", "ozonolysis", "hydrohalogenation", "electrophil"],
  5: ["nmr", "ir", "spectroscop", "synthesis", "retrosynthe", "mass spec", "review", "infrared"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/nmr|ir spectroscop|synthesis|retrosynthe/i.test(t)) return 5;
  if (/alkene|addition|alkyne|markovnikov/i.test(t)) return 4;
  if (/sn1|sn2|e1|e2|substitution|elimination/i.test(t)) return 3;
  if (/alkane|chiral|stereo|conformation/i.test(t)) return 2;
  return 1;
}
