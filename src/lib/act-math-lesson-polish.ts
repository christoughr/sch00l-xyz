/** Sort and polish ACT Math publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/practice|exam|strategy|pacing|calculator|plug-in|test day/i.test(t))
    return 5;
  if (/statistic|probability|mean|median|counting/i.test(t)) return 4;
  if (/geometry|triangle|circle|trigonometry|trig|sohcahtoa/i.test(t))
    return 3;
  if (/algebra|equation|quadratic|function|system/i.test(t)) return 2;
  if (/fraction|decimal|percent|ratio|number|integer/i.test(t)) return 1;
  return 1;
}
