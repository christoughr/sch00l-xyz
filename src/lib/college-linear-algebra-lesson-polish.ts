/** Sort and polish College Linear Algebra publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["matrix", "rref", "gaussian", "row operation", "inverse", "linear system"],
  2: ["vector space", "basis", "dimension", "span", "independence", "subspace", "rank", "nullity"],
  3: ["linear transformation", "kernel", "image", "change of basis", "column space"],
  4: ["determinant", "eigenvalue", "eigenvector", "diagonaliz", "characteristic"],
  5: ["orthogonal", "gram-schmidt", "least squares", "projection", "review"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/eigen|determinant/i.test(t)) return 4;
  if (/transformation|kernel/i.test(t)) return 3;
  if (/basis|dimension|span/i.test(t)) return 2;
  return 1;
}
