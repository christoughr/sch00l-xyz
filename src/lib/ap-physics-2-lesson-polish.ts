/** Sort and polish AP Physics 2 publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/practice|exam|frq|mcq|lab report|review/i.test(t)) return 5;
  if (/optic|lens|quantum|nuclear|interference/i.test(t)) return 4;
  if (/magnet|induction|rc circuit|circuit/i.test(t)) return 3;
  if (/capacitor|potential|electrostatic|gauss/i.test(t)) return 2;
  if (/fluid|pressure|thermo|entropy|buoyancy/i.test(t)) return 1;
  return 1;
}
