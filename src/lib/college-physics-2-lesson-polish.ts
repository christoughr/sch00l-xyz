/** Sort and polish College Physics II (E&M) publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["coulomb", "electric field", "charge", "gauss", "electrostatic", "flux"],
  2: ["potential", "capacitor", "dielectric", "voltage", "energy density"],
  3: ["current", "resistance", "circuit", "kirchhoff", "ohm", "rc"],
  4: ["magnetic", "ampere", "faraday", "induction", "solenoid", "lenz"],
  5: ["inductor", "ac circuit", "maxwell", "electromagnetic", "review"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/magnetic|induction|faraday/i.test(t)) return 4;
  if (/circuit|current|resistance/i.test(t)) return 3;
  if (/potential|capacitor/i.test(t)) return 2;
  if (/electric field|coulomb/i.test(t)) return 1;
  return 1;
}
