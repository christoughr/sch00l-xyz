/** Sort and polish ACT English publisher lessons. */
...
function scoreUnitFallback(text: string): number {
  ...
  if (/practice|exam|strategy|pacing|test day|annotation|timing/i.test(t))
    return 5;
  if (/writing|thesis|introduction|conclusion|revise|development/i.test(t))
    return 4;
  if (/reading|comprehension|main idea|inference|vocabulary/i.test(t))
    return 3;
  if (/rhetoric|rhetorical|concision|transition|word choice|tone/i.test(t))
    return 2;
  if (/grammar|punctuation|comma|agreement|pronoun|tense|mechanics/i.test(t))
    return 1;
  return 1;
}
