/** Sort and polish Discrete Mathematics publisher lessons. */
...
const UNIT_KEYWORDS: Record<number, string[]> = {
  1: ["logic", "proof", "proposition", "quantifier", "induction", "contradiction", "truth table"],
  2: ["set", "function", "relation", "injective", "surjective", "bijection", "cardinality"],
  3: ["combinatorics", "permutation", "combination", "binomial", "counting", "pigeonhole", "inclusion"],
  4: ["graph", "vertex", "edge", "tree", "euler", "hamilton", "path", "cycle"],
  5: ["recurrence", "algorithm", "complexity", "induction", "recursive", "big-o", "review"],
};
...
function scoreUnitFallback(text: string): number {
  ...
  if (/recurrence|algorithm|complexity/i.test(t)) return 5;
  if (/graph|tree|euler/i.test(t)) return 4;
  if (/combinatorics|permutation|combination/i.test(t)) return 3;
  if (/set|function|relation/i.test(t)) return 2;
  return 1;
}
