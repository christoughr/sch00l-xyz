...
import { detectSuspiciousExtension } from "@/lib/extension-guard";
 ...
 * This deters casual copying, screen capture in-tab, extension bypass tools, and multi-monitor signals.
 */
export function ContentShield({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const extInterval = setInterval(() => {
      const hit = detectSuspiciousExtension();
      if (hit) {
        setBlocked(true);
        setReason(`${hit} Disable copy/print extensions to view lessons.`);
      }
    }, 3000);
