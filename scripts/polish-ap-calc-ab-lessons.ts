#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-ap-calc-ab-lessons.ts — after AP Calc AB Paste 3 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-calc-ab-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run 024_seed_ap_calc_ab_course.sql first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → AP Calculus AB");
}
