#!/usr/bin/env npx tsx
/**
 * Redistribute AP Bio publisher lessons + clean titles.
 * npx tsx scripts/polish-ap-bio-lessons.ts
 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-bio-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) process.exit(1);
  ...
  const publisherIds = rows
    .filter((l) => isPublisherLesson(l.source_pdf_name))
    .map((l) => l.id);

  console.log("Bump ord slots...");
  let bumpOrd = 6000;
  ...
  console.log("Done — refresh https://sch00l.ai/study → AP Biology");
}
