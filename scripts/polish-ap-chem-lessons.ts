#!/usr/bin/env npx tsx
/**
 * Redistribute AP Chem publisher lessons + clean titles.
 * npx tsx scripts/polish-ap-chem-lessons.ts
 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-chem-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run 021_seed_ap_chem_course.sql first");
    process.exit(1);
  }
  ...
  console.log("Done — refresh https://sch00l.ai/study → AP Chemistry");
}
