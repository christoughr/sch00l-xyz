#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-sat-reading-lessons.ts — after SAT Reading publish */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/sat-reading-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run seed-sat-reading.mjs first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → SAT Reading & Writing");
}
