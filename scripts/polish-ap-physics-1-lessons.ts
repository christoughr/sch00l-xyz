#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-ap-physics-1-lessons.ts — after AP Physics 1 Paste 3 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-physics-1-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run 025_seed_ap_physics_1_course.sql first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → AP Physics 1");
}
