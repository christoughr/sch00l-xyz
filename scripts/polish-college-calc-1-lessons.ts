#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-college-calc-1-lessons.ts — after College Calc I Paste 3 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/college-calc-1-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run scripts/seed-college-calc-1.mjs first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → Calculus I");
}
