#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-college-calc-2-lessons.ts — after College Calc II publish */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/college-calc-2-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run scripts/seed-college-calc-2.mjs first");
    process.exit(1);
  }
  ...
  if (publisherUpdates.length === 0) {
    console.log("Applying", openStaxUpdates.length, "seed cleanups only...");
  } else {
  ...
  console.log("Done — https://sch00l.ai/study → Calculus II");
}
