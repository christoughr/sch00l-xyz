#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-ap-stats-lessons.ts — after AP Statistics publish */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-stats-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run seed-ap-stats.mjs first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → AP Statistics");
}
