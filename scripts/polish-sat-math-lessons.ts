#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-sat-math-lessons.ts — run in terminal after SAT Paste 3 */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/sat-math-lesson-polish";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const TRACK = "sat-math";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run 022_seed_sat_math_course.sql first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → SAT Math");
}
