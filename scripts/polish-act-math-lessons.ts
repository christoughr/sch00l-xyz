#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-act-math-lessons.ts — after ACT Math publish */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/act-math-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run seed-act-math.mjs first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → ACT Math");
}
