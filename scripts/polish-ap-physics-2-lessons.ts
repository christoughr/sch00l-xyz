#!/usr/bin/env npx tsx
/** npx tsx scripts/polish-ap-physics-2-lessons.ts — after AP Physics 2 publish */
import fs from "fs";
...
import {
  ...
  isPublisherLesson,
  type LessonRow,
} from "../src/lib/ap-physics-2-lesson-polish";
...
async function main() {
  ...
  if (!units?.length) {
    console.error("Run seed-ap-physics-2.mjs first");
    process.exit(1);
  }
  ...
  console.log("Done — https://sch00l.ai/study → AP Physics 2");
}
